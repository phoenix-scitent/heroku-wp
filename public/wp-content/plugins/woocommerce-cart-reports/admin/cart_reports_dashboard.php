...
public function setup_widgets() {
	...
	$cart_heading .= '<a href="index.php?wc_cart_month='.($current_month_offset-1).'" class="previous">&larr; '.date_i18n('F', strtotime('01-'.($the_month_num-1).'-2011')).'</a><span>'.__('Monthly Carts', 'woocommerce_cart_reports').$tooltip.'</span>';

	if(function_exists('WC')) {
		if(current_user_can('edit_shop_orders')) {
	       wp_add_dashboard_widget( 'woocommerce_dashboard_carts_right_now', __( 'Recent Cart Activity', 'woocommerce_cart_reports' ), array($this,'woocommerce_recent_cart_activity' ));       
	    }
	}
	else {
		if(current_user_can('edit_shop_orders') ){
	       wp_add_dashboard_widget( 'woocommerce_dashboard_carts_right_now', __( 'Recent Cart Activity', 'woocommerce_cart_reports' ), array($this,'woocommerce_recent_cart_activity_legacy' ));       
	    }
...
<div class="table table_carts_right_now">
	...
	<div class="table table_carts">
		<p class="sub woocommerce_sub"><?php _e( "The Last $days Day" . $plural, 'woocommerce_cart_reports' ); ?></p>
		<table>
			<tr class="first">
...
<ul class="wc_status_list">
	...
	<?php
		$num  = number_format_i18n( $nums_right_now['Abandoned']  ); //We're adding these together for the "Not Converted" field.
		$text =  __( sprintf("carts abandoned over the last %s Day%s", $days, $plural), 'woocommerce_cart_reports' );
		$link = add_query_arg( array( 'post_type' => 'carts', 'mv' => 'Abandoned','start_date' => date('Y-m-d', time() - (60*60*24 * $days)), 'end_date' => date('Y-m-d', time()) ), get_admin_url( null, 'edit.php' ) );
	?>
	<a href="<?php echo $link; ?>">
		<strong>
