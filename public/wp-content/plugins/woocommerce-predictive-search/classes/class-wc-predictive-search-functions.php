...
class WC_Predictive_Search_Functions
{
	...
	public static function change_add_to_cart_url( $url, $product ) {
		if ( 'simple' == $product->product_type ) {
			...
			$url            = $product->is_purchasable() && $product->is_in_stock() ? remove_query_arg( 'added-to-cart', add_query_arg( array_merge( array( 'variation_id' => $product->variation_id, 'add-to-cart' => $product->id ), $variation_data ), get_permalink( $product->id ) ) ) : get_permalink( $product->id );
		}
