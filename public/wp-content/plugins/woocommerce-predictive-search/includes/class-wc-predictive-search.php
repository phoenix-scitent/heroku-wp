...
class WC_Predictive_Search
{
	...
	public function get_product_categories_nested( $parent = 0, $child_space = '', $cats_excluded = NULL ) {
		...
		$results_p_categories = get_terms( array(
		...
		if ( $results_p_categories ) {
			foreach( $results_p_categories as $p_categories_data ) {
				...
				$categories_list[$p_categories_data->term_id] = array(
					'name' => $child_space . $p_categories_data->name,
					'slug' => $p_categories_data->slug,
					'url'  => get_term_link( $p_categories_data->slug, 'product_cat' )
				);

				$child_categories_list = $this->get_product_categories_nested( $p_categories_data->term_id, $child_space . '&nbsp;&nbsp;&nbsp;', $cats_excluded );

				$categories_list = array_merge( $categories_list, $child_categories_list );
			}
		}
