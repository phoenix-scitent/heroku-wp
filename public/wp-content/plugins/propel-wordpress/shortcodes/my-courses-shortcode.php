...
wp_register_style('courselist-css', plugins_url( '/css/courselist.eqcss.css', __FILE__ ) );
wp_register_script('list-js', plugins_url( '/js/list.js', __FILE__ ), '' , '', true );
wp_register_script('list-fuzzysearch-js', plugins_url( '/js/list.fuzzysearch.min.js', __FILE__ ), '' , '', true );
wp_register_script('semantic-popup-js', plugins_url( '/js/semantic-popup.js', __FILE__ ));
wp_register_script('courselist-js', plugins_url( '/js/courselist.js', __FILE__ ), '' , '', true );
wp_register_script('dotdotdot','https://cdnjs.cloudflare.com/ajax/libs/jQuery.dotdotdot/1.7.4/jquery.dotdotdot.min.js');
...
function propel_my_courses_shortcode( $attr ) {
  ...
  $keys = $wpdb->get_results( "
              ...
              " );
  echo '<div class="courselist-container" id="courselist-container">';
  ?>
  ...
  <div class="list-filters">
  Filter:
  <select id="filter-courselist-list" class="sort-filter">
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;
  Sort: 
  <select id="sort-courselist-list" class="sort-filter">
...
function propel_render_my_courses_list($key) {
  ...
  wp_enqueue_style( 'courselist-css' );
  wp_enqueue_script('list-js');
  wp_enqueue_script('list-fuzzysearch-js');
  wp_enqueue_script('semantic-popup-js');
  wp_enqueue_script('dotdotdot');
  wp_enqueue_script('courselist-js');
...
function propel_render_course_access_button( $post_id, $percentage, $active, $expires ) {
  if ($active == false) {
    ...
    return "<a class='course-access progress-$percentage push-bottom expired' role='button' href='#'>Expired on $expired_on</a>";
  }
	if ($percentage == 0) {
  ...
  if (sizeof($lesson_ids) == 1) {
	  ...
	  $xapi_id = $wpdb->get_var( "
                          ...
                          ;" );
	  if ($xapi_id != 0) {
			error_log("propel_render_course_access_button " . $xapi_id);
      // <a class="grassblade_launch_link" href="http://cleanstart.scitent.us/wp-content/uploads/grassblade/6239-proposal-writing/index.html?actor=%7B%22mbox%22%3A%22mailto%3Atjohnson%40scitent.us%22%2C%22name%22%3A%22Tim%20Johnson%22%2C%22objectType%22%3A%22Agent%22%7D&amp;auth=Basic%20MzMtYzA5ZTE3OGU1ZWY4MDdmOmY0NTZkYjEwMTc1MGQ4MTVhMTIyNzIyZGM%3D&amp;endpoint=http%3A%2F%2Flrs.propellms.com%2FxAPI%2F&amp;registration=&amp;activity_id=http%3A%2F%2Fcareerdevelopment.aaas.org%2Fcourse1" target="_blank">Launch</a>
		  $url = do_shortcode("[grassblade text='$button_label' id=".$xapi_id." target='url']");
      $extraclasses = $extraclasses . " grassblade_launch_link";
      $linktarget = "_blank";
		} else {
			error_log("propel_render_course_access_button NO EMBED");
		}
  ...
  // If the above didnt work, embed the old button to go to the course page
	return "<a class='course-access progress-$percentage act-btn push-bottom$extraclasses' role='button' href='". $url ."' target='". $linktarget ."'>$button_label</a>";
}
