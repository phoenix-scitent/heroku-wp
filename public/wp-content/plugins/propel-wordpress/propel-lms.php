...
include 'propel-userpro-helper.php';
include 'propel-ajax.php';
include 'propel-emails.php';
  ...
  function auth0_user_logged_in($user_id, $user_profile, $is_new, $id_token, $access_token) {
    ...
    function render_propel_okm() {
      ...
      if ( $settings['okm_sso_enabled'] == 'on' && $a0UserId) {
        ...
        $out = '<iframe src="' . Propel_LMS::okm_server() . '/accounts/sso/' . $tenant_key . '" width="' . $width . '" height="' . $height . '" frameborder="0" scrolling="auto"></iframe>';
        echo $out;
      } else {
