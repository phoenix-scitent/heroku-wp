...
class CP_Admin_Welcome {
	...
	public function admin_menus() {
		...
		switch ( $_GET['page'] ) {
			case 'cp-about' :
				$page = add_dashboard_page( $welcome_page_title, $welcome_page_name, 'manage_options', 'cp-about', array( $this, 'about_screen' ) );
				break;
			case 'cp-shortcode' :
				$page = add_dashboard_page( $welcome_page_title, $welcome_page_name, 'manage_options', 'cp-shortcode', array( $this, 'shortcode_screen' ) );
				break;
			case 'cp-faqs' :
			 	$page = add_dashboard_page( $welcome_page_title, $welcome_page_name, 'manage_options', 'cp-faqs', array( $this, 'faqs_screen' ) );
				break;
		}
	}
	...
	public function admin_head() {
		...
		remove_submenu_page( 'index.php', 'cp-shortcode' );
		remove_submenu_page( 'index.php', 'cp-faqs' );
	...
	private function intro() {
		...
		<div class="feature-section col two-col"><br>
			<div class="col-1">
				<p class="woocommerce-actions">
					<a href="<?php echo admin_url('post-new.php?post_type=product'); ?>" class="button button-primary"><?php _e( 'Create combo!', SA_WC_Chained_Products::$text_domain ); ?></a>
		...
		<h2 class="nav-tab-wrapper">
			<a class="nav-tab <?php if ( $_GET['page'] == 'cp-about' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'cp-about' ), 'index.php' ) ) ); ?>">
				<?php _e( "Know Chained Products", SA_WC_Chained_Products::$text_domain ); ?>
			</a>
			<a class="nav-tab <?php if ( $_GET['page'] == 'cp-shortcode' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'cp-shortcode' ), 'index.php' ) ) ); ?>">
				<?php _e( "Shortcode", SA_WC_Chained_Products::$text_domain ); ?>
			</a>
			<a class="nav-tab <?php if ( $_GET['page'] == 'cp-faqs' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'cp-faqs' ), 'index.php' ) ) ); ?>">
	...
	public function about_screen() {
		...
		<?php $this->intro(); ?>
			...
			<div>
				...
				<div class="feature-section col three-col" >
					<div class="col">
						<h4><?php echo __( 'WooCommerce Product Bundle', SA_WC_Chained_Products::$text_domain ); ?></h4>
						<p>
							<?php echo __( 'Chained Products works with WooCommerce Product Bundle. You can also set a Chained Parent as an item in Product Bundle. Whenever the product bundle will be added to cart or order, chained parent along with its chained item will also be added to cart or order.', SA_WC_Chained_Products::$text_domain ); ?>
						</p>
					</div>
					<div class="col">
						<h4><?php echo __( 'WooCommerce Give Products', SA_WC_Chained_Products::$text_domain ); ?></h4>
						<p>
							<?php echo __( 'Chained Products also works with WooCommerce Give Products. Whenever store admin will give a chained perent to his/her customer, all its chained products will also be given automatically.', SA_WC_Chained_Products::$text_domain ); ?>
						</p>
					</div>
					<div class="col last-feature">
	...
	public function faqs_screen() {
		...
		<div class="clear"></div>
		<div align="center">
			<p><?php echo sprintf(__( 'If you are facing any other issues, either search on %s & if not found submit a ticket from there itself.', SA_WC_Chained_Products::$text_domain ), '<a href="https://support.woothemes.com/">' . __( 'WooThemes', SA_WC_Chained_Products::$text_domain ) . '</a>' ); ?></p>
		</div>
	...
	public function cp_welcome() {
		...
		wp_redirect( admin_url( 'index.php?page=cp-about' ) );
		exit;
