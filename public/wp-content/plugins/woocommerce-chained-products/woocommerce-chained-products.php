...
if ( is_woocommerce_active() ) {
	...
	if ( in_array('woocommerce-product-bundles/woocommerce-product-bundles.php', $active_plugins) || array_key_exists('woocommerce-product-bundles/woocommerce-product-bundles.php', $active_plugins) ) {
		add_action( 'woocommerce_add_to_cart', 'add_bundled_chained_products_to_cart', 99, 6 );
		add_action( 'woocommerce_cart_item_restored', 'restore_bundled_chained_products_in_cart', 99, 2 );
	}
	...
	function woocommerce_chained_products_for_variable_product() {
		...
		$children = ( Chained_Products_WC_Compatibility::is_wc_gte_24() ) ? $product->children['visible'] : $product->children;
	...
	/**
	 * Function to add chained product of those product which are part of product bundles
	 * 
	 * @global SA_WC_Chained_Products $wc_chained_products Main instance of Chained Products
	 * @param string $cart_item_key
	 * @param int $product_id
	 ...
	 * @param array $cart_item_data
	 */
	function add_bundled_chained_products_to_cart( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
		global $wc_chained_products;
		...
		foreach ( $cart_contents as $cart_id => $cart_item ) {
			...
			if ( $cart_item['bundled_by'] != $cart_item_key ) continue;

			$product_id 	= $cart_item['product_id'];
			$quantity 		= $cart_item['quantity'];

			// Variation ID & Variation is not passed because only Simple products can be added as child in Product Bundle
			add_chained_products_to_cart( $cart_id, $product_id, $quantity, 0, array(), array() );
	...
	/**
	 * Function to restore chained products of Product Bundles
	 * 
	 * @global SA_WC_Chained_Products $wc_chained_products Main instance of Chained Products
	 * @param string $cart_item_key
	 * @param WC_Cart $cart
	 */
	function restore_bundled_chained_products_in_cart( $cart_item_key, $cart ) {
		global $wc_chained_products;
		...
		foreach ( $cart_contents as $cart_id => $cart_item ) {
			...
			if ( empty( $cart_item['bundled_by'] ) ) continue;
			if ( $cart_id != $cart_item_key ) continue;
	...
	/**
	 * Function to add chained items for product bundles
	 * 
	 * @param array $cart_item
	 * @param string $cart_item_key
	 * @return array $cart_item
	 */
	function add_chained_items_for_bundles( $cart_item = null, $cart_item_key = null ) {

		global $wc_chained_products;

		if ( empty( $cart_item ) ) return $cart_item;

		if ( ! empty( $cart_item['stamp'] ) && ! empty( $cart_item['bundled_by'] ) ) {
			
			$product_ids = array_keys( $cart_item['stamp'] );

			if ( empty( $product_ids ) ) return $cart_item;

			foreach ( $product_ids as $product_id ) {
				$chained_products_detail= $wc_chained_products->get_all_chained_product_details( $product_id );
				$quantity = ( ! empty( $cart_item['stamp'][$product_id]['quantity'] ) ) ? $cart_item['stamp'][$product_id]['quantity'] : 1;
				if ( ! empty( $chained_products_detail ) ) {
					add_chained_products_to_cart( $cart_item_key, $product_id, $quantity, 0, array(), array() );
				}
			}
