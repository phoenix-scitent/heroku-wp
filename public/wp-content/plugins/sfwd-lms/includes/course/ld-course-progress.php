...
/**
 * LFF 10/05/15 : Listen for grassblade filter that displays links to
 * launch quizzes. If the quiz is not accessable, replace the link with a message
 */
add_filter( 'grassblade_shortcode_return', 'grassblade_test',1,4 );
function grassblade_test($return, $params, $shortcode_atts, $attr ) {
	global $post;
	if ($post->post_type == "sfwd-quiz") {
		error_log("This is a quiz... see if it is accessabile");
		if ( is_quiz_accessable() ) {
			return $return;
		}
		$return = "<h4 style='margin:10px 0 40px 0;color:#666;'>Please go back and complete the previous lessons.</h4>";
	} 
	return $return;
}
