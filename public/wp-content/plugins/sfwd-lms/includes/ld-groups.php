 ...
 *
 * @return array  list of group leaders
 */
	...
	<script language="Javascript">
		jQuery(document).ready(function() {
			jQuery("#publish").on("click",function() {
				if (jQuery("#learndash_group_users").length) {
					jQuery('#learndash_group_users option').each(function(){
	...
	<div id="learndash_groups_page_box" class="learndash_groups_page_box">
			...
			if ( ! empty( $all_group_leaders ) ) : ?>
				<table>
					<tr>
						...
						<td align="center">
							<a href="#" onClick="SelectAddRows(document.getElementById('learndash_group_leaders_search'), document.getElementById('learndash_group_leaders')); SelectAll(document.getElementById('learndash_group_leaders')); return false;"><img src="<?php echo plugins_url( "/assets/images/arrow_right.png", dirname( __FILE__ ) ); ?>" /></a><br>
							<a href="#" onClick="SelectRemoveRows(document.getElementById('learndash_group_leaders')); SelectAll(document.getElementById('learndash_group_leaders')); return false;"><img src="<?php echo plugins_url("assets/images/arrow_left.png", dirname( __FILE__ ) ); ?>" /></a>
						</td>
		...
		<table>
			<tr>
				...
				<td align="center">
					<a href="#" onClick="SelectAddRows(document.getElementById('learndash_group_users_view'), document.getElementById('learndash_group_users')); SelectAll(document.getElementById('learndash_group_users')); return false;"><img src="<?php echo plugins_url("assets/images/arrow_right.png", dirname( __FILE__ ) ); ?>" /></a><br>
					<a href="#" onClick="SelectRemoveRows(document.getElementById('learndash_group_users')); SelectAll(document.getElementById('learndash_group_users')); return false;"><img src="<?php echo plugins_url("assets/images/arrow_left.png", dirname( __FILE__ ) ); ?>" /></a>
				</td>
...
function learndash_groups_save_postdata( $post_id ) {
	...
	if ( is_numeric( $learndash_group_enroll_course ) ) {
		$learndash_group_enroll_course = array( $learndash_group_enroll_course );
	}

	if ( is_numeric( $learndash_group_unenroll_course ) ) {
		$learndash_group_unenroll_course = array( $learndash_group_unenroll_course );
	}
...
function learndash_group_user_list( $group_id ) {
	...
	if ( ! empty( $users ) ) {
		$sql = "SELECT * FROM $wpdb->users WHERE `ID` IN (" . implode( ',', $users ) . ')';
		$sql .= ' AND ID NOT IN (SELECT user_id FROM `' . $wpdb->prefix . "usermeta` where meta_key = 'wp_capabilities' AND (meta_value like '%\"administrator\"%' OR meta_value like '%\"group_leader\"%'))";
		$users = $wpdb->get_results( $sql );
 ...
 * 
 * @param  int 		$group_id
 * @return array 	list of users that belong to group
 */
 ...
 * 
 * @param  int 		$group_id
 * @return array 	list of admin id's
 */
...
function learndash_group_admin_menu() {
	add_submenu_page( 
		...
		'group_leader', 
		'group_admin_page', 
		'learndash_group_admin_menu_page' 
	);
}
