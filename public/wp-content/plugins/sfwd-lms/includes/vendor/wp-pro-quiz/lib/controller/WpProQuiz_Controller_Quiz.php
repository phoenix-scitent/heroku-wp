...
// 				break;
	...
	public function isLockQuiz($quizId) {
		...
		if(1) { // added by uday for prerequisite lessons
			$wdm_quiz_meta = get_post_meta( learndash_get_quiz_id_by_pro_quiz_id($quiz->getId()), '_sfwd_wdm_pre_lessons', true );
			
			if( $wdm_quiz_meta != '' ) {
				$prerequisite_lessons = unserialize( base64_decode( $wdm_quiz_meta ) ); // because lessons quiz meta saved in base64 encoded format
				if( !  empty( $prerequisite_lessons ) ) {

					$wdm_lessons_names = array();
					foreach( $prerequisite_lessons as $wdm_val ) {
						error_log("Pre-requisite lesson $wdm_val");
						if( learndash_is_lesson_notcomplete( get_current_user_id(),  array( $wdm_val => 0 ) ) ) { // need to send in array format like this.
							array_push($wdm_lessons_names, htmlspecialchars_decode(get_the_title( $wdm_val )) );
						}
					}
					$wdm_lessons_names = array_filter($wdm_lessons_names);
					if( !  empty( $wdm_lessons_names ) )
						$data['prerequisiteLesson'] = implode( ', ', $wdm_lessons_names );
				}
			}
