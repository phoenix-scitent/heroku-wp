...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View {
	...
	public function show($preview = false) {
		...
		<?php 
			...
			$this->showPrerequisiteLessonsBox(); // added by uday
	...
	public function script($preview = false) {
		...
		// ==========================================================
		// THIS STRIPS THE CORRECT ANSWERS OUT OF THE JSON
		// FORCES AJAX, AND BREAKS WDMCHECKER.
		//
...
<?php 
	...
	private function showPrerequisiteLessonsBox() {  // added by uday
		?>
		<div style="display: none;" class="wpProQuiz_prerequisite_lessons">
			<p>
				<?php _e('You have to finish following lesson(s), to start this quiz:', 'wp-pro-quiz'); ?> 
				<span></span>
			</p>
		</div>
		<?php 
					...
					<div class="wpProQuiz_question" style="margin: 10px 0px 0px 0px;">
						...
						<?php
							...
							foreach($answerArray as $v) {
								...
								$answer_hint = esc_html($v->getAnswerHint());
								if($answer_text == '') {
									continue;
								}
	...
	// 									$catPoints[$question->getCategoryId()] += $question->getPoints();
							...
							<?php if($question->getAnswerType() === 'single' || $question->getAnswerType() === 'multiple') { ?>
								...
								<?php /* $json[$question->getId()]['coordinates'][] = (int)$v->isCorrect(); */ ?>
									...
									<label>
										...
										<!-- contains specific answer hint for each answer in single choice questions. -->
										<input type="hidden" id="question_<?php echo $this->quiz->getId(); ?>_<?php echo $question->getId(); ?>_<?php echo ($answer_index+1); ?>" value="<?php echo $answer_hint; ?>" />
									</label>
							
							<?php } else if($question->getAnswerType() === 'sort_answer') { ?>
								<?php
									// LFF: Changed this to place answer text in the correct array. This
									//      is needed to make the new check and contine mode work 
									$json[$question->getId()]['correct'][] = $answer_text; /*(int)$answer_index;*/ ?>
					...
					<?php } ?>
						...
						<input type="button" name="wdmcheck" value="<?php _e('Check', 'wp-pro-quiz'); ?>" class="wpProQuiz_button wpProQuiz_QuestionButton" style="float: right; display: none;" >
