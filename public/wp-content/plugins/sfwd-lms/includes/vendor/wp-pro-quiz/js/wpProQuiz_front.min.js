(function($) {
	...
	$.wpProQuizFront = function(element, options) {
		...
		var quizStatus = {
			...
			loadLock: 0,
			isPrerequisite: 0,
			isPrerequisiteLesson: 0, // added by uday
			isUserStartLocked: 0,
		};
		...
		plugin.methode = {
			...
			*/
				...
				list.each(function() {
					...
					if (wdmresult.c) {
						if (name == 'singleMulti') {
							...
							if (!wdmresult.r.trim()) {
								$this.find('.wdmProQuiz_response').hide();
								...
								$this.find('.wdmProQuiz_incorrect').hide();
								$this.find('.wdmProQuiz_correct .wdmmessage').text(wdmresult.r);
								$this.find('.wdmProQuiz_correct').show();
								
							}
							$questionList.find('.wpProQuiz_questionInput').attr('disabled', 'disabled');
						}
						
						else if (name == 'sort_answer' || name == 'free_answer' || name == 'matrix_sort_answer' || name == 'cloze_answer') {
							$this.find('.wpProQuiz_response').show();
							$this.find('.wpProQuiz_incorrect').hide();
							$this.find('.wpProQuiz_correct').show();
						}
						
						if (name == 'assessment_answer') {
							$this.find('.wpProQuiz_response').show();
							$this.find('.wpProQuiz_correct').show();
						}
						
						$this.find(globalNames.wdmcheck).hide();
						...
						if (name == 'singleMulti') {
							...
							if (!wdmresult.r.trim()) {
								...
								$this.find('.wpProQuiz_response').hide();
								$this.find('.wdmProQuiz_response').show();
								$this.find('.wdmProQuiz_correct').hide();
								$this.find('.wdmProQuiz_incorrect .wdmmessage').text(wdmresult.r);
								$this.find('.wdmProQuiz_incorrect').show();
							}
						}
						
						else if (name == 'cloze_answer') {
							$this.find('.wpProQuiz_response').show();
							$this.find('.wpProQuiz_incorrect').show();
							$this.find('.wpProQuiz_correct').hide();
						}
						
						else if (name == 'sort_answer' || name == 'free_answer' || name == 'matrix_sort_answer') {
							$this.find('.wpProQuiz_response').show();
							$this.find('.wpProQuiz_incorrect').show();
							$this.find('.wpProQuiz_correct').hide();
						}
			...
			startQuiz: function(loadData) {
				...
				if(quizStatus.isPrerequisiteLesson) { // added by uday
					globalElements.quizStartPage.hide();
					$e.find('.wpProQuiz_prerequisite_lessons').show();
...
//						$questionList.sortable("destroy");
			...
			checkQuizLock: function() {
				...
				plugin.methode.ajax({
                    ...
                    if(json.prerequisiteLesson != undefined) { // added by uday
						quizStatus.isPrerequisiteLesson = 1;
						$e.find('.wpProQuiz_prerequisite_lessons span').html(unescape(json.prerequisiteLesson));
					}
